...
function Abar_loaded()
	...
	fbar_VL()
		...
		getglobal(fbar_mh:GetName()..Border):Hide()
		getglobal(fbar_oh:GetName()..Border):Hide()
		...
		getglobal(fbar_mh:GetName()..Bordern):Show()
		getglobal(fbar_oh:GetName()..Bordern):Show()
		...
		getglobal(fbar_mh:GetName()..Bordern):Hide()
		getglobal(fbar_oh:GetName()..Bordern):Hide()
		getglobal(Abar_Mhr:GetName()..Border):Hide()
		getglobal(Abar_Oh:GetName()..Border):Hide()
		getglobal(ebar_mh:GetName()..Border):Hide()
		getglobal(ebar_oh:GetName()..Border):Hide()
		...
		getglobal(fbar_mh:GetName()..Border):Show()
		getglobal(fbar_oh:GetName()..Border):Show()
		end
end
...
function fbar_VL()
	fbar_mh:SetPoint("LEFT",fbar_Frame,"TOPLEFT",6,-13)
	fbar_oh:SetPoint("LEFT",fbar_Frame,"TOPLEFT",6,-35)
	fbar_mhText:SetJustifyH("Left")
	fbar_ohText:SetJustifyH("Left")
end
function fbar_event(event)
	if event=="VARIABLES_LOADED" then
	fbar_VL()
	end
end

function fbar_set(focus)
	eons,eoffs = UnitAttackSpeed("focus")
--[[	
	Mob duel weild code.... I cant get their off hand speed to return
	...
	if ((math.abs((eont-epont)-eons) <= math.abs((eofft-epofft)-eoffs))and not(eonh <= eoffs/eons)) or eoffh >= eons/eoffs then
		...
		eonh = eonh +1
		eons = eons - math.fmod(eons,0.01)
		fbar_mhs(eons,"Target Main["..eons.."s]",1,.1,.1)
	else
		epofft = eofft
		eoffh = eoffh+1
		eonh = 0
		eohd,eold = ohd-math.fmod(eohd,1),old-math.fmod(eold,1)
		eoffs = eoffs - math.fmod(eoffs,0.01)
		fbar_ohs(eoffs,"Target Off["..eoffs.."s]",1,.1,.1)
	end
	else ]]--
	eons = eons - math.fmod(eons,0.01)
	fbar_mhs(eons,"focus".."["..eons.."s]",1,.1,.1)
	--end
end

function fbar_mhs(bartime,text,r,g,b)
fbar_mh:Hide()
if abar.info == true then
	fbar_mh.txt = text
	fbar_mhText:SetText(text)
else
	fbar_mh.txt = ""
	fbar_mhText:SetText("")
end
fbar_mh.st = GetTime()
fbar_mh.et = GetTime() + bartime
fbar_mh:SetStatusBarColor(r,g,b)
fbar_mh:SetMinMaxValues(fbar_mh.st,fbar_mh.et)
fbar_mh:SetValue(fbar_mh.st)
fbar_mh:Show()
end
function fbar_ohs(bartime,text,r,g,b)
fbar_oh:Hide()
fbar_oh.txt = text
fbar_oh.st = GetTime()
fbar_oh.et = GetTime() + bartime
fbar_oh:SetStatusBarColor(r,g,b)
fbar_ohText:SetText(text)
fbar_oh:SetMinMaxValues(fbar_oh.st,fbar_oh.et)
fbar_oh:SetValue(fbar_oh.st)
fbar_oh:Show()
end
